...
def correlation_heatmap(df):
    numerical_columns = ['AGE', 'CHILD_TOTAL', 'PERSONAL_INCOME', 'TARGET', 
                         'LOAN_NUM_TOTAL', 'LOAN_NUM_CLOSED', 'OWN_AUTO']
...
def loans_scatter(df):
    fig  = px.scatter(df, x='LOAN_NUM_TOTAL', y='LOAN_NUM_CLOSED',
                      title = 'Количество взятых кредитов и \
                        количество возвращенных кредитов',
                      labels = dict(x = 'Взятые кредиты', 
                                    y = 'Возвращенные кредиты'),
