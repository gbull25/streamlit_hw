...
st.header('Информация о клиентах')
st.divider()
...
with st.expander('Сколько зарабатывает семья клиента?', expanded=True):
    ...
    st.write('Семейный бюджет большинства клиентов составляет от 10 до 50 тысяч рублей.')
st.divider()
...
st.write('На графике видно сильную положительную корреляцию\
          ...
          подсветить зависимость между признаками количество\
          детей и возраст клиента, а также между индвидуальным заработком\
